...
class Backend {
  navigateGet (req, res, next) {
    ...
    if (link !== undefined) { res.writeHead('200', { 'Link': link.split('\n') }) }
    if (content !== undefined) { res.write(content) } else { res.writeHead('404', []) }
    res.end()
  }
